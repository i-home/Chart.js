...
var element_arc = core_element.extend({
	...
	draw: function() {
		...
		if (core_plugins.notify(me, 'beforeArcDraw', [args]) === false) {
			return;
		}
		ctx.save();
		...
		ctx.restore();
		core_plugins.notify(me, 'afterArcDraw', [args]);
	}
});
...
var element_point = core_element.extend({
	...
	draw: function(chartArea) {
		...
		if (chartArea === undefined || helpers$1.canvas._isPointInArea(vm, chartArea)) {
			...
			if (core_plugins.notify(me, 'beforePointDraw', [args]) === false) {
				return;
			}
			helpers$1.canvas.drawPoint(ctx, pointStyle, radius, x, y, rotation);
			core_plugins.notify(me, 'afterPointDraw', [args]);
		}
	}
});
...
var Legend = core_element.extend({
	...
	fit: function () {
		...
		if (isHorizontal) {
			...
			var padding = 0; try { padding = opts.layout.padding; } catch (ignore) { }			minSize.height += totalHeight + padding;
			...
			columnHeights.push(currentColHeight);
			var padding = 0; try { padding = opts.layout.padding; } catch (ignore) { }			minSize.width += totalWidth + padding;
		}
